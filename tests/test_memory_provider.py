...
def test_agent_loop_accepts_none_memory_provider():
    """Test that AgentLoop accepts memory_provider=None (for fallback testing)."""
    from pathlib import Path
    from unittest.mock import MagicMock
    from superbot.agent.loop import AgentLoop
    from superbot.bus.queue import MessageBus
    ...
    # Create AgentLoop without memory_provider
    loop = AgentLoop(
        bus=mock_bus,
        provider=mock_provider,
    ...
    # Verify memory_provider is None
    assert loop.memory_provider is None
    # In _consolidate_memory, it will fallback to self.provider


def test_make_provider_with_local_model_enabled():
    """Test that when local_model enabled, memory_provider is created."""
    # Note: We can't fully test MLX provider creation without a real model path,
    # but we can verify the logic path is taken
    config = Config(
        agents=AgentsConfig(defaults=AgentDefaults(model="test-model", provider="minimax")),
        local_model=LocalModelConfig(enabled=True, provider="mlx", path="/fake/path"),
    )
    main_provider, memory_provider = _make_provider(config)

    # main_provider should still be created from agents.defaults.provider
    assert main_provider is not None
    # memory_provider should be created from local_model
    assert memory_provider is not None, "memory_provider should be created when local_model is enabled"
